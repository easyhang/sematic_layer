...
在问数问答场景中，核心挑战在于**数据层与用户实际问题之间存在语义鸿沟**：

- 用户提出的问题是业务语言（"上周华东区毛利率下滑的原因是什么？"），而数据层返回的是结构化字段和 SQL 结果集。
- LLM 虽然擅长理解自然语言，却不知道如何将业务问题映射到具体的取数算子（表、字段、过滤条件、聚合逻辑、时间窗口）上。

**语义层的角色就是充当翻译层**，在用户问题与底层数据之间建立可被稳定调用的映射：将业务概念落地为 L0 原子实体和 SQL 模板，让 LLM 能够沿"用户问题 → 业务概念 → 原子实体 → SQL → 数据事实 → 业务回答"的路径可靠地执行问数问答，而不是凭猜测拼凑 SQL。

![智能问数流程](.github/smart-query-flow.png)
...
![语义层工作过程](.github/semantic-layer-workflow.png)
...
Skill 会审核开发者输入是否满足语义层构建要求，识别缺失信息并给出追问建议：

![Skill 输入审核](.github/skill-input-review.png)
